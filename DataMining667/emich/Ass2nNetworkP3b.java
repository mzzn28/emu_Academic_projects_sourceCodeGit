	...
	//method reads inputs from input file and writes outputs to output file
		...
		{ 
			...
			int numberRecords = inFile.nextInt(); 
				...
				{ 
					...
					for (int j = 0; j < numberOutputs; j++) {
								double lablel=deNormalize(output[j]);
		...
		{ 
			...
			for (int i = 0; i < numberRecords; i++) 
			{
				...
				{
					double value=inFile.nextDouble();
					actualOutput[j] = normalize(value);
				}
				...
				//print o/p
				//for (int j = 0; j < numberOutputs; j++){
					//System.out.print("predicted:"+ deNormalize(predictedOutput[j]));
					//System.out.println(" , Actual:"+	deNormalize(actualOutput[j]));}
			...
			//find average error 
			System.out.println("Error: " + (error/numberRecords)*100); 
			inFile.close();
		}
...
/***************************************************************/ 
	//method to normalize inputs and outputs hard coded for s & p

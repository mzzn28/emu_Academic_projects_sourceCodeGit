...
public class ClusterKmeanAss3POne2 {
...
/*************************************************************************/ 
	...
	{ 
		...
		for(int i=0; i<centroids.size();i++)
		{
			...
			for(int j=0; j<numberAttributes;j++){
				cent=Math.round( centroids.get(i).attributes[j] * 1000.0)/1000.0;
...
/**********************************************************/
	...
	{
		//print last updated centroid
		for(int i=0; i<centroids.size();i++)
		{
			System.out.println("C"+ (i+1) +":");
			for(int j=0; j<numberAttributes;j++)
				System.out.print(centroids.get(i).attributes[j] + " ");;
				System.out.println();
		}
		
	}
	public void sumSquarError()
	{
		//sum squared Error
		double sum=0;
			for (int i = 0; i < numberRecords; i++) 	
			{
				double dist=distance(records.get(i),centroids.get(clusters[i]));
				sum += (dist*dist);
			}

...
/*************************************************************************/ 		 
	...
	//method converts attribute values to normalized [0-1], hard coded for 
	 //specific problem -1 grades of student
	private double convertIn(String label, int column) 
	{
		 double value=0;
		 ...
		 //convert third column grades ABC [0, 1]
		 else
		 { 
			 if (label.equals("A"))
				 value=0.9;
	...
	//method converts output values to normalized [0-1], hard coded for 
	//specific problem -1 grades of student
	private double convertOut(String label, int column) 
	{
		double value=-1;
	...
	//method that revert back o/p normalize value to to original class value
		private String revertOut(double value){
			...
			if(errrorPercenta<errrorPercentb && errrorPercenta< errrorPercentc )
				outClass="bad";
			if(errrorPercentb<errrorPercentc && errrorPercentb< errrorPercenta )
				outClass="average";
			if(errrorPercentc<errrorPercentb && errrorPercentc< errrorPercenta )
				outClass="good";
...
/******************************************************************************/	
	...
	{ 
		...
		for (int i = 0; i < numberRecords; i++) 
		{
			...
			{
				String label=inFile.next();
				actualOutput[j] = convertOut(label, j+1);
			}
			//find predicted output 
...
/**********************************************************************/ 	
	...
	private double computeErrorMatch(double[] actualOutput, double[] predictedOutput) 
	{
		...
		for (int i = 0; i < actualOutput.length; i++)
		{ 
			//find the %age eeor b/w both outputs
	...
	{
		//weights of i/p and hidden nodes
		System.out.println(" i/p and hidden:");
		for (int i = 0; i < numberInputs; i++) {
			for (int j = 0; j < numberMiddle; j++) {
		...
		//weights of o/p and hidden nodes
		System.out.println(" o/p and hidden:");
		for (int i = 0; i < numberMiddle; i++) {
			for (int j = 0; j < numberOutputs; j++) {
		...
		//theth's of middle layer
		System.out.println(" Theta of middle nodes:");
		for (int i = 0; i < numberMiddle; i++) {
				System.out.println( thetaMiddle[i] + "  ");
		}
		
		
		//theth's of o/p layer
		System.out.println();
		System.out.println(" Theta of o/p node:");
		for (int i = 0; i < numberOutputs; i++) {
				System.out.println( thetaOut[i] + "  ");
		}
	} 
